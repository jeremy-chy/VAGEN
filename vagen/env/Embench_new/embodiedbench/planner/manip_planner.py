...
class ManipPlanner():
    ...
    ##### add era-related code #####

    def get_response_from_url(self, message):
        try:
            payload = {"message": message}
            response = requests.post("http://127.0.0.1:5000/respond", json=payload, timeout=60)
    ...
    def get_message_era(self, images, instruction, object_info, interaction_history):
        current_message = [
            {
                "role": "system",
                "content": [
                    {"type": "text", "text": self.system_prompt.format(VOXEL_SIZE, VOXEL_SIZE, int(360 / ROTATION_RESOLUTION), ROTATION_RESOLUTION)}],
            },
            {
                "role": "user",
                "content": [
    ...
    def output_to_action(self, output_text):
        ...
        action_list = [int(x.strip()) for x in action_match.group(1).split(',')] if action_match else []
        return [action_list], think_text
        ...
        if self.visual_icl and not self.language_only:
            ...
            if len(self.episode_messages) == 0:
                ...
                if self.chat_history:
                    ...
                    self.episode_messages = self.get_message_visual_icl(obs, first_prompt, task_prompt, task_variation)
        elif self.model_type == "era":
            self.episode_messages = self.get_message_era(obs, user_instruction, avg_obj_coord, self.episode_act_feedback)
            print("Input Message:\n", self.episode_messages)
        else:
    ...
    def update_info(self, info, thinking_output):
        env_feedback = info['env_feedback']
        action = info['action']
        self.episode_act_feedback.append({
            "step_id": self.planner_steps,
